...
variable "vps_boot_disk_type" {
  description = "Boot disk type used by the RC single-vps preset."
  type        = string
  default     = "pd-balanced"
}

variable "vps_use_static_public_ip" {
  description = "Whether the RC single-vps preset should reserve a static public IPv4 address."
  type        = bool
  default     = true
}
...
variable "api_container_limits" {
  description = "Container resource limits for the RC Cloud Run API."
  type        = map(string)
  default = {
    cpu    = "1"
    memory = "512Mi"
