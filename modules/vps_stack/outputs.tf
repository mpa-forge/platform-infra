...
output "public_ip" {
  ...
  value       = try(coalesce(google_compute_address.public[0].address, google_compute_instance.this[0].network_interface[0].access_config[0].nat_ip), null)
}
...
output "frontend_url" {
  ...
  value       = try("http://${coalesce(google_compute_address.public[0].address, google_compute_instance.this[0].network_interface[0].access_config[0].nat_ip)}${local.frontend_suffix}", null)
}

output "backend_url" {
  description = "Derived backend URL for the single-VPS preset."
  value       = try("http://${coalesce(google_compute_address.public[0].address, google_compute_instance.this[0].network_interface[0].access_config[0].nat_ip)}${local.backend_suffix}", null)
}
...
output "runtime_contract" {
  ...
  value = {
    ...
    static_public_ip_enabled  = var.use_static_public_ip
    public_ip                 = try(coalesce(google_compute_address.public[0].address, google_compute_instance.this[0].network_interface[0].access_config[0].nat_ip), null)
    frontend_url              = try("http://${coalesce(google_compute_address.public[0].address, google_compute_instance.this[0].network_interface[0].access_config[0].nat_ip)}${local.frontend_suffix}", null)
    backend_url               = try("http://${coalesce(google_compute_address.public[0].address, google_compute_instance.this[0].network_interface[0].access_config[0].nat_ip)}${local.backend_suffix}", null)
